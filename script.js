document.addEventListener("DOMContentLoaded", function () {
    ...
    menuIcon.addEventListener("click", function () {
        if (mobileMenu.classList.contains("show")) {
            hideMobileMenu(mobileMenu, header);
        } else {
    ...
    function hideMobileMenu(mobileMenu, header) {
    ...
    window.addEventListener('resize', () => hideMobileMenu(mobileMenu, header));

    // logic for scrolling down to "Cases" and "Contacts" sections
    const casesButton = document.getElementById("casesButton");
    const casesSection = document.getElementById("casesSection");
    activateScrollToTargetSection(casesButton, casesSection, false);
    const contactsButton = document.getElementById("contactsButton");
    const footerSection = document.getElementById("footerSection");
    activateScrollToTargetSection(contactsButton, footerSection, false);
    
    const mobileCasesButton = document.getElementById("mobileCasesButton");
    activateScrollToTargetSection(mobileCasesButton, casesSection, true);
    const mobileContactsButton = document.getElementById("mobileContactsButton");
    activateScrollToTargetSection(mobileContactsButton, footerSection, true);

    function activateScrollToTargetSection(button, targetSection, needToHideMobileMenu) {
        button.addEventListener("click", function () {
            targetSection.scrollIntoView({
                behavior: "smooth",
                block: "start"
            });
            if (needToHideMobileMenu) {
                hideMobileMenu(mobileMenu, header);
            }
        });
    }
